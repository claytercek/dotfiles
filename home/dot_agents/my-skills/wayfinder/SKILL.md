...
**Where the map, its child tickets, blocking, and frontier queries physically live is tracker-specific.** Resolve the **issue tracker** and **triage label** from your project config (the `## Agent project config` block in `AGENTS.md` / `CLAUDE.md`). If either is absent, stop and ask the user — and suggest they run `/setup-my-skills` to persist it. Never assume a default.
